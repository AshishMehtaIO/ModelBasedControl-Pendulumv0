...
class Planner(object):
    ...
    def __init__(self, model_prediction):
        ...
        self._theta_dot_bins = 360
        self._theta_dot_grid = np.linspace(start=-8, stop=8, num=self._theta_dot_bins, dtype=np.float32)
        self._theta_dot_grid_2d = np.repeat(self._theta_grid.reshape(1, -1), self._action_bins, axis=0)
    ...
    def compute_heuristic(state_action):
        ...
        state_square[:, 1] = 0.01 * state_square[:, 1]
    ...
    def find_path(self, start_state, goal_state):
        ...
        print("Goal State: {}".format(self._goal_node))
